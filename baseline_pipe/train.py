"""
The train.py of baseline_pipe: 
1. model is the CycleGAN from train_3'.
"""""
...
from baseline_pipe.utils import ReplayBuffer, Logger, weights_init_normal
from baseline_pipe.models import Generator, Discriminator
from baseline_pipe.datasets import ImageDataset

os.environ['CUDA_VISIBLE_DEVICES'] = '0'

# Hyperparameters
Hyperparameters = {"input_c_A": 1, "output_c_A": 1, "input_c_B": 1, "output_c_B": 1, "lr": 2e-4, "batch_size": 1,
...
def train_model(netG_A2B, netG_B2A, netD_A, netD_B, criterion_GAN, criterion_cycle, criterion_identity,
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            loss_identity_B = criterion_identity(same_B, real_B) * 5.0 # the identity loss is always used with
                                                                    # a weighting half that of the cycle loss (5-times),
                                                                    # matching the official implementation source code.
            ...
            # Total loss
            loss_G_A2B = loss_identity_B + loss_GAN_A2B + loss_cycle_BAB
            loss_G_B2A = loss_identity_A + loss_GAN_B2A + loss_cycle_ABA
            ...
            # Total loss
            loss_D_A = (loss_D_real + loss_D_fake) * 0.5 # a weighting it used so that updates to
                                                        # the model have half (0.5) the usual effect.
            loss_D_A.backward()
            ...
            # Total loss
            loss_D_B = (loss_D_real + loss_D_fake) * 0.5 # a weighting it used so that updates to
                                                        # the model have half (0.5) the usual effect.
            loss_D_B.backward()
        ...
        if save_models:
            ...
            torch.save(netG_A2B.state_dict(), f"/home/changxiao/CX/code/CycleGAN_MRI/baseline_pipe/output/netG_A2B_BatchSize={key_word}.pth")
            torch.save(netG_B2A.state_dict(), f"/home/changxiao/CX/code/CycleGAN_MRI/baseline_pipe/output/netG_B2A_BatchSize={key_word}.pth")
            torch.save(netD_A.state_dict(), f"/home/changxiao/CX/code/CycleGAN_MRI/baseline_pipe/output/netD_A_BatchSize={key_word}.pth")
            torch.save(netD_B.state_dict(), f"/home/changxiao/CX/code/CycleGAN_MRI/baseline_pipe/output/netD_B_BatchSize={key_word}.pth")

        # # Save models checkpoints
        # last_time_loss_list = [loss_G_A2B, loss_G_B2A, loss_D_A, loss_D_B]
        # if save_models:
        #     if epoch == 0:
        #         torch.save(netG_A2B.state_dict(),
        #                    f"baseline_pipe/output/netG_A2B_epoch={epoch}.pth")
        #         torch.save(netG_B2A.state_dict(),
        #                    f"baseline_pipe/output/netG_B2A_epoch={epoch}.pth")
        #         torch.save(netD_A.state_dict(),
        #                    f"baseline_pipe/output/netD_A_epoch={epoch}.pth")
        #         torch.save(netD_B.state_dict(),
        #                    f"baseline_pipe/output/netD_B_epoch={epoch}.pth")
        #     elif epoch > 0:
        #         if loss_G_A2B < last_time_loss_list[0]:
        #             torch.save(netG_A2B.state_dict(),
        #                        f"baseline_pipe/output/netG_A2B_epoch={epoch}.pth")
        #         if loss_G_B2A < last_time_loss_list[1]:
        #             torch.save(netG_B2A.state_dict(),
        #                        f"baseline_pipe/output/netG_B2A_epoch={epoch}.pth")
        #         if loss_D_A < last_time_loss_list[2]:
        #             torch.save(netD_A.state_dict(),
        #                        f"baseline_pipe/output/netD_A_epoch={epoch}.pth")
        #         if loss_D_B < last_time_loss_list[3]:
        #             torch.save(netD_B.state_dict(),
        #                        f"baseline_pipe/output/netD_B_epoch={epoch}.pth")
...
def train(pretrained):
    ...
    if pretrained:
        ...
        netG_A2B.load_state_dict(torch.load(f"baseline_pipe/output/netG_A2B_epoch=.pth"))
        netG_B2A.load_state_dict(torch.load(f"baseline_pipe/output/netG_B2A_epoch=.pth"))
        netD_A.load_state_dict(torch.load(f"baseline_pipe/output/netD_A_epoch=.pth"))
        netD_B.load_state_dict(torch.load(f"baseline_pipe/output/netD_B_epoch=.pth"))
    else:
