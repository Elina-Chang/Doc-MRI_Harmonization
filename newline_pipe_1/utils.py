...
def compute_total_variation_loss(image, weight = 2e-5):
...
class Logger():
    def __init__(self, n_epochs, batches_epoch):
        self.viz = Visdom(env="train_modified_01")
            ...
            for loss_name, loss in self.losses.items():
                if loss_name not in self.loss_windows:
                    self.loss_windows[loss_name] = self.viz.line(X=np.array([self.epoch]),
                                                                 Y=np.array([float(loss) / self.batch]),
                                                                 opts={'xlabel': 'epochs', 'ylabel': loss_name,
                                                                       'title': loss_name})
                else:
                    self.viz.line(X=np.array([self.epoch]), Y=np.array([float(loss) / self.batch]),
...
if __name__ == "__main__":
    # test
    data = torch.randn(56, 56)
    plt.imshow(data, cmap="gray")
    plt.title("original data")
    # plt.show()
    data1 = ReplayBuffer().push_and_pop(data)
    plt.imshow(data1, cmap="gray")
    plt.title("poped data")
    # plt.show()

    print(torch.eq(data, data1))
