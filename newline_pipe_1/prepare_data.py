"""
Used in my_dataset for MyDataset, for preparing the needed data
"""
...
def find_modal(cases_path):
    ...
    for case_path in cases_path:
        if os.path.isdir(case_path):
            if len(os.listdir(case_path)) == 9:
                sag = glob.glob(case_path + r"/*sag.nii")
                modals_path.extend(sag)
            elif len(os.listdir(case_path)) == 10:
                sagResample = glob.glob(case_path + r"/*sag_Resample.nii")
                modals_path.extend(sagResample)
    return sorted(modals_path)


def get_path(root_dir=r"/home/changxiao/CX/data", dir1="GE", dir2="Philips"):
    """
    Return all needed image paths
    :param root_dir:
    :param dir1:
    :param dir2:
    :return:
    """
    cases_dir1 = glob.glob(os.path.join(root_dir, dir1) + "/*")
    cases_dir2 = glob.glob(os.path.join(root_dir, dir2) + "/*")
    modals_dir1 = np.squeeze(find_modal(cases_dir1))
    modals_dir2 = np.squeeze(find_modal(cases_dir2))

    return modals_dir1, modals_dir2


def get_abnormal_case(path):
    """
    # Acquire the cases' names of the abnormal data
    :param path: .excel file path for storing the abnormal info
    :return: (set)形式: {}
    """
    csv = pd.read_excel(path, index_col=None)
...
def get_selected_path(root_dir=r"/home/changxiao/CX/data", dir1="GE", dir2="Philips",
                      only_3T=True, GE_csv_path="/home/changxiao/CX/code/cyclegan_mri/GE_data_info.csv",
                      elinimate_abnormal=False, abnormal_cases_csv=r"/home/changxiao/CX/code/"
                                                                  r"cyclegan_mri/Abnormal Data.xlsx"):
    """
    ...
    GE_data_info = pd.read_csv(GE_csv_path, index_col=False)
    ...
    if elinimate_abnormal:
        # 获取异常cases
        GE_abnormal_cases, Philips_abnormal_cases = get_abnormal_case(path=abnormal_cases_csv)  # 请注意，这里是以集合形式存储
...
def DataPreprocessing(img_data, zeropadding=False, target_size=512, center_crop2D=True, crop_size=280):
    ...
    # Define a new_normalization_on_GE to [0,1]
    img_data = (img_data - img_data.min()) / (img_data.max() - img_data.min()).astype(np.float32)
