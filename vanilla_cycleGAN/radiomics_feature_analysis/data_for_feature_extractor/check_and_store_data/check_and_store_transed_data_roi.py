...
def examine_and_store_data(img_root_path, roi_root_path, store_path):
    cases_path_img = glob.glob(img_root_path + "\*")[18:]
    cases_path_roi = glob.glob(roi_root_path + "\*")[18:]
    for case_img, case_roi in zip(cases_path_img, cases_path_roi):
        print(os.path.basename(case_img))

        img_path = glob.glob(case_img + "\*t2sag_ori.nii")[0]
...
if __name__ == "__main__":
    examine_and_store_data(img_root_path=r"E:\PhD\Data_renji\Data_forFeatureExtractor\GE_3T",
                           roi_root_path=r"E:\PhD\Data_renji\Data_forFeatureExtractor\GE_3T",
