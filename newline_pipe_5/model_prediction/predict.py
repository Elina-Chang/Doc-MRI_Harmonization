...
from newline_pipe_5.models import Generator
from Tools.normalization1.generate_clip_value import generate_clip_value


def get_data_path(root_dir=r""):
    case_path_list = glob.glob(root_dir + "\*")
    data_path_list = sorted([glob.glob(case_path + "\*t2sag.nii.gz") for case_path in case_path_list])
    data_path_list = np.squeeze(data_path_list).tolist()
    return data_path_list[37:]
...
def predict(pth_root_path=r"G:\PhD\PycharmProjects\my_project\MRI_Harmonization\newline_pipe_5"
                          r"\output\Norm1_39SIEMENS&7UIH_and_75Philips\checkpoints\150"):
    Hyperparameters = {"input_c_A": 1, "output_c_A": 1, "input_c_B": 1, "output_c_B": 1, "batch_size": 8,
                       "num_epochs": 150}
    ...
    netG_A2B.load_state_dict(
        torch.load(os.path.join(pth_root_path, f"netG_A2B_BatchSize={batch_size}_epoch={epoch}.pth.tar")))
    ...
    GE_modals_list = get_data_path(root_dir=r"G:\PhD\Data_renji\Data_3T_Resampled_Norm1\SIEMENS_3T_Resampled_Norm1")
    for GE_modal in GE_modals_list:
        ...
        store_path = os.path.join(r"G:\PhD\StyleTransferPredictions\newline_pipe_5\39SIEMENSpredictions\Epoch150",
                                  GE_modal.split("\\")[5])
